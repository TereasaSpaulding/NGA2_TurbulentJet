...
module lpt_react_class
 ...
 contains
    ...
    subroutine advance(this,dt,U,V,W,rho,visc,stress_x,stress_y,stress_z,vortx,vorty,vortz,T,srcU,srcV,srcW,srcE)
       ...
       type(part) :: myp,pold 
       real(WP) :: cp, m   ! ADDED: mass, specific heat and heat flux for particle       
       real(WP) :: Q
       !real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_) :: fT
       ...
       do i=1,this%np_
          ...
          do while (dt_done.lt.dt)
             ...
             Ip = 0.1_WP*myp%d**2
             ! ADDED
             m = 100.0_WP*((4.0_WP/3.0_WP) *Pi * (0.5_WP*myp%d)**3)
             cp = 1005.0_WP            
             ...
             call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,T=T,p=myp,acc=acc,torque=torque,opt_dt=myp%dt, Q=Q)
             ...
             ! ADDED
             myp%temp=pold%temp+0.5_WP*mydt*Q/(m*cp)
             ...
             ! ADDED
             myp%temp=pold%temp+mydt*Q/(m*cp)
             ! DEBUGGING
             !print *, myp%temp
             ! Relocalize
    ...
    subroutine get_rhs(this,U,V,W,rho,visc,stress_x,stress_y,stress_z,T,p,acc,torque,opt_dt,Q)
       implicit none
       class(lpt_react), intent(inout) :: this
       real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
       real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
       ...
       compute_heat_transfer: block
          ...
          use mathtools, only: Pi
          real(WP) :: k, A  ! Conduction parameters
          call param_read('Conduction Coeff',k)
          ! Surface area assuming spherical particle
          A = 4*Pi*(0.5_WP*p%d)**2         
          ! Heat flux to particle
          Q = k*A*(fT - p%temp)/(0.5_WP*p%d)
